...
func Server(keyFunc jwt.Keyfunc, opts ...Option) middleware.Middleware {
	o := &options{
		...
		claims:        func() jwt.Claims { return &AuthClaims{} },
	}
...
func GetUsername(ctx context.Context) (username string, err error) {
	...
	if claims, ok := FromContext(ctx); !ok {
		//
		return "", errors.BadRequest("no login info", "无法获取登录用户信息")
	} else {
		ac, ok := claims.(*AuthClaims)
		if !ok {
			return "", errors.InternalServer("claims transfer error", "claims 转换失败")
		}
